...
const CategoryList: React.FC<IResourceComponentsProps> = ({ initialData }) => {
        ...
        >
          <div>
            {/* <Avatar
              ...
              icon={<CustomIcon type="DragOutlined" />}
            /> */}
  ...
  const moveListItem = async (dragIndex, hoverIndex, updateParent = true) => {
    ...
    // Update the 'sort' attribute based on the new order
    ...
    if (dragItem.id != dropItem.id) {
      await directusClient.items("category").updateOne(dragItem.id, {
        sort: hoverIndex + 1,
      });
      await directusClient.items("category").updateOne(dropItem.id, {
        sort: dragIndex + 1,
      });
      formQuery.refetch();
    }
  ...
  return (
    <>
      ...
      {/* <List style={{ padding: 10 }} itemLayout="horizontal"
      {...listProps}
        dataSource={items}
        renderItem={(item:any, index:any) => {
            ...
            return (
                    ...
                    <DraggableListItem item={{ id: item.id, name: item.name }} index={index} moveListItem={moveListItem} />
